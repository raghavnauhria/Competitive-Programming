...
void solve(int r, int s, vpi &ans)//, string &check)
{
	if(r < 2)
	{
		return;
	...
	{
		ans.pb(mp( (s-1)*r - i , r-1));
		
		// string a = check.substr(0, (s-1)*r - i);
		// string b = check.substr((s-1)*r - i, r-1);
		// // cout << a sp b ;
		// check.replace(0, r-1, b);
		// check.replace(r-1, (s-1)*r - i, a);
	}
	solve(r-1, s, ans);//, check);
}
