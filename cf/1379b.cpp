...
int main()
{
	...
	{
		cinlll(l, r, m);
		ll upper = m+(r-l), lower = m-(r-l);

		if(m > r)
		{
			if(lower <= r)
			{
				cout << lower sp r sp l << endl; 
			}
			else
			{
				ll num = lower + l - lower % l;
				num -= m;

				if(num >= 0)
					cout << l sp l sp (l+num) << endl;
				else
					cout << l sp r sp (r+num) << endl;
			}
		}
		else if(m < l)
		{
			cout << upper sp l sp r << endl;
		}
		else
		{
			cout << m sp m sp m << endl;
		}
