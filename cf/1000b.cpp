...
int main()
{
	...
	vl prefix(n+2, 0);
	prefix[1] = a[0]; // on-time

	fi(i, 2, n+2)
		prefix[i] += prefix[i-2] + (a[i-1]-a[i-2]);
	// print(prefix);

	ll on = prefix[n+1-n%2], off = prefix[n+n%2];
	ans = on;
	
	if(a[0] > 1)
	{
		ans = max(ans, a[0]-1+off);
	}
	...
	fi(i, 1, n+1)
	{
		if(a[i] - a[i-1] > 1)
		{
			if(i&1) // off-section
			{
				ans = max(ans, prefix[i] + (a[i]-a[i-1]-1) + (off-prefix[i+1]));
			}
			else
			{
				ans = max(ans, prefix[i+1]-1 + (off-prefix[i]));
			}
		}
	}
