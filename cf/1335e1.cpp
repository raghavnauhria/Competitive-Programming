...
int main()
{
	...
	while(t--)
	{
		...
		vector<vector<int> > prefix(27, vector<int> (n, 0) );
		prefix[a[0]][0]++;
		...
		fi(i, 0, n)
		{
				...
				ans = max(ans, prefix[a][i]);
		}

		fd(i, n-1, 1)
		{
			fi(a, 1, 27)
				ans = max(ans, prefix[a][n-1] - prefix[a][i-1]);
		}
		...
		fi(i, 1, n-1)
		{
			fi(j, i, n-1)
			{
				...
				{
					int y = prefix[b][j] - prefix[b][i-1];
					fi(a, 1, 27)
					{
						int x = min(prefix[a][i-1], prefix[a][n-1] - prefix[a][j]);

						// if(i == 2 && j == 6 && b == 2 && a == 1)
						// 	cout << "Look " << x sp y << endl;
