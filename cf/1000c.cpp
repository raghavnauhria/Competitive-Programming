...
#define print(v) for (auto it = v.begin()+1; it != v.end(); ++it) cout << *it << ' '; cout << endl;
...
int main()
{
	...
	{
		int l = lower_bound(all(cval), a[i].first) - cval.begin();
		int r = lower_bound(all(cval), a[i].second+1) - cval.begin();
		++count[l];
		--count[r];
	}
