...
#define cinvs(v, n) vs v(n, 0); fi(i, 0, n) cin >> v[i];
...
int main()
{
	...
	while(t--)
	{
		cini(n);
		string s;
		unordered_map<string, bitset<26> > tab;

		fi(i, 0, n)
		{
			cin >> s;
			tab[s.substr(1)][s[0]-'a'] = 1;
		}
		...
		{
			for(auto j = next(i); j != tab.end(); ++j)
			{
				bitset<26> temp = (i->second ^ j->second);
				ans += ( (temp & i->second).count() * (temp & j->second).count() );
			}
		}
